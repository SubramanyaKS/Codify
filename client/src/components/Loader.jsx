    ...
    <div className={`${asOverlay ? 'inset-0 fixed' : 'py-24'} flex flex-col items-center justify-center z-[9999] ${isDark ? 'bg-black/90' : 'bg-light-bg-primary/80'} backdrop-blur-sm`}>
      ...
      <div className="relative w-32 h-32 mb-6">
        <div className="absolute inset-0 flex items-end justify-center">
          <div className="w-8 h-16 bg-primary rounded-t-sm transform -rotate-12 animate-pulse"></div>
        ...
        <div className="absolute bottom-8 left-4 w-1 h-1 bg-primary/50 rounded-full animate-float animation-delay-500"></div>
        <div className="absolute top-8 right-8 w-1.5 h-1.5 bg-primary/80 rounded-full animate-float animation-delay-700"></div>
      </div>

      {/* Enhanced Text Animation */}
      <div className="flex items-center justify-center font-righteous text-4xl mb-4">
        <span className={`inline-block animate-bounce-slow animation-delay-100 ${isDark ? 'text-white' : 'text-primary'} transform hover:scale-110 transition-transform`}>C</span>
        <span className={`inline-block animate-bounce-slow animation-delay-200 ${isDark ? 'text-white' : 'text-primary'} transform hover:scale-110 transition-transform`}>o</span>
        <span className={`inline-block animate-bounce-slow animation-delay-300 ${isDark ? 'text-white' : 'text-primary'} transform hover:scale-110 transition-transform`}>d</span>
        <span className={`inline-block animate-bounce-slow animation-delay-400 ${isDark ? 'text-white' : 'text-primary'} transform hover:scale-110 transition-transform`}>i</span>
        <span className={`inline-block animate-bounce-slow animation-delay-500 ${isDark ? 'text-white' : 'text-primary'} transform hover:scale-110 transition-transform`}>f</span>
        <span className={`inline-block animate-bounce-slow animation-delay-600 ${isDark ? 'text-white' : 'text-primary'} transform hover:scale-110 transition-transform`}>y</span>
      </div>

      {/* Study Progress Bar */}
      <div className={`w-48 h-2 ${isDark ? 'bg-white/20' : 'bg-primary/20'} rounded-full overflow-hidden mb-3`}>
        <div className={`h-full ${isDark ? 'bg-white' : 'bg-primary'} rounded-full animate-progress`}></div>
      </div>

      {/* Loading Text with Study Context */}
      <div className={`${isDark ? 'text-white/80' : 'text-primary/80'} text-sm font-medium animate-pulse`}>
